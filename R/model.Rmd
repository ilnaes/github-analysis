...
lin_res <- lin_workflow %>%
  tune_grid(folds,
    ...
    grid = crossing(
      ...
      max_tokens = floor(seq(100, 1000, length.out = 5))
    )
  )
...
* best: 0.684
* max_tokens: 1000 (but 700 looks good enough)
...
```{r}
doParallel::registerDoParallel(cores = 1) # need to do this or training hangs for some reason
