...
About what you would expect for the Github crowd.  Web friendly languages on top, scientific computing on bottom.
...
Also skewed, but log transform seems to be sufficient.  There were some two repos with size 0:

```{r}
df |> filter(size == 0)
```

One was removed due to regulation and one was probably an error since nuwa is there.
...
The scientific computation languages are different from the general programming languages in that they have more projects with less (or 0) issues.  This makes sense as analyses do not typically generate issues.
...
Organizations tend to have more stars.  This is especially pronounced in C++ and Typescript.  Visually, it looks less pronounced in Matlab and R.
...
Users are far more likely to have no license or use MIT
...
License has some weak correspondence with popularity, although *not* having a license seems to have a stronger correspondence to having low popularity.
