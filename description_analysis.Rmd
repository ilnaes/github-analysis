...
We will try to construct a model that predicts which matches a description to a language.  We will use a KNN model with cosine similarity as a metric.

```{r}
cos_rec <- recipe(language ~ description, data = df) |>
  step_mutate(
    description = str_replace_all(description, "[^\\x00-\\x7F]", " "),
    description = str_replace_all(description, "\\s+", " "),
    nwords = str_count(description, "\\w+")
...
res <- cos_workflow |>
  tune_grid(folds,
    ...
    control = control,
    grid = crossing(
      neighbors = floor(seq(50, 250, length.out = 50)),
      max_tokens = floor(seq(50, 350, length.out = 50)),
    )
  )

res <- saveRDS(res, "res.rds")
```
